...
(function () {
  ...
  function normalizeItems(items) {
    ...
    items.forEach(function (item, idx) {
      ...
      if (item.type === "table") {
        ...
        if (rows.length === 1) {
          ...
          rows.forEach(function (r) {
            r.forEach(function (cellText) {
              cards.push(parseCardText(cellText));
            });
          });
      ...
      if (isStep(text)) {
        blocks.push({ kind: "step", text: text });
        return;
      }

      if (isHeading(text)) {
        blocks.push({ kind: "heading", text: text });
        return;
      }
  ...
  function renderCard(container, card) {
    ...
    if (card.title) {
      c.appendChild(el("div", "card-title", escapeHtml(card.title)));
    }
    ...
    blocks.forEach(function (b) {
      switch (b.kind) {
        ...
        case "step":
          ...
          if (!stepMatch) {
            badge.classList.add("step-badge-closing");
          }

          var label = el(
            "div",
            "step-text",
            escapeHtml(b.text.replace(/^(PASO\s+\d+|CIERRE)\s*·?\s*/i, ""))
          );
  ...
  function panelHero(container, opts) {
    ...
    if (opts.phase) {
      hero.appendChild(el("div", "phase-label", escapeHtml(opts.phase)));
    }

    if (opts.eyebrow) {
      hero.appendChild(el("div", "eyebrow", escapeHtml(opts.eyebrow)));
    }

    hero.appendChild(el("h2", "", escapeHtml(opts.title)));

    if (opts.number !== undefined) {
      hero.appendChild(el("div", "panel-number", opts.number));
    }

    if (opts.tagline) {
      hero.appendChild(el("div", "panel-tagline", escapeHtml(opts.tagline)));
    }
  ...
  function buildPortada(container) {
    ...
    meta.innerHTML =
      "Elaborado especialmente para <strong>" +
      escapeHtml(state.name) +
      "</strong><br>Fecha de nacimiento: " +
      pad2(state.day) +
      "/" +
      pad2(state.month) +
      "/" +
  ...
  function buildCodigo(container, code) {
    ...
    if (items) {
      ...
      container.appendChild(
        el("p", "block-p", "Contenido no disponible para este número todavía.")
      );
    }
  }
  ...
  function buildCierre(container) {
    ...
    box.innerHTML =
      "<p>Lleva contigo tu Mapa completo, listo para imprimir y releer cuantas veces tu corazón lo necesite.</p>";

    var btn = el(
      "button",
      "btn-download",
      "⬇️ Descargar Mi Mapa Personalizado (PDF)"
    );
    ...
    container.appendChild(
      el(
        "div",
        "footer-note",
        "Con profundo amor · Tere Becerra Huerta · Creadora del Código Maestro de Vida™"
      )
    );
  }
  ...
  function launchApp() {
    ...
    if (progressTrack) {
      ...
      TABS.forEach(function () {
        progressTrack.appendChild(el("div", "progress-seg"));
      });
    }
    ...
    TABS.forEach(function (tab, idx) {
      ...
      if (idx !== TABS.length - 1) {
        ...
        nextBtn.addEventListener("click", function () {
          goToTab(Math.min(TABS.length - 1, idx + 1));
        });
  ...
  function pushOutOfLine(y, lineBreaks) {
    for (var i = 0; i < lineBreaks.length; i++) {
      ...
      if (
        y > lb.top - SAFE_CUT_MARGIN &&
        y < lb.bottom + SAFE_CUT_MARGIN
      ) {
  ...
  function renderSectionToPdf(
    ...
    while (renderedY < canvas.height && guard < 500) {
      ...
      var remainingOnPage =
        pageHeightPx - pdfState.usedHeightPx;

      var desiredEnd = Math.min(
        renderedY + remainingOnPage,
        canvas.height
      );
      ...
      pdfState.usedHeightPt +=
        sliceHeightPt;

      pdfState.usedHeightPx +=
        sliceHeightPx;

      renderedY = safeEnd;

      if (
        window.__PDF_AUDIT__ &&
        lineBreaks &&
        lineBreaks.length
      ) {
        var safeEndDomCheck =
          safeEnd / sectionScale;

        for (
          var ai = 0;
          ai < lineBreaks.length;
          ai++
        ) {
          var alb = lineBreaks[ai];

          if (
            safeEndDomCheck > alb.top + 0.5 &&
            safeEndDomCheck < alb.bottom - 0.5
          ) {
            console.log(
              "[AUDIT_FAIL] safeEnd cae DENTRO de una linea! safeEndDom=" +
                safeEndDomCheck.toFixed(2) +
                " line.top=" +
                alb.top +
                " line.bottom=" +
                alb.bottom +
  ...
  function generatePdf(iosTabName) {
    ...
    return fontsReady
      ...
      .then(function () {
        var sectionBreakRects =
          sections.map(function (s) {
            return collectBreakRects(s);
          });

        var sectionLineBreaks =
          sections.map(function (s) {
            return collectLineBreaks(s);
          });
  ...
  /* ---------------------------------------------------------
     GARANTÍA DE LIBRERÍAS EXTERNAS
     --------------------------------------------------------- */
  var LIB_SOURCES = {
    html2canvas: [
    ...
    return Promise.all(tasks).then(
      function () {
        if (
          ...
          throw new Error(
            "Las herramientas para generar el PDF no cargaron correctamente. Revisa tu conexión a internet e intenta de nuevo."
          );
        }
      }
    );
  ...
  function handleDownload() {
    ...
    ensurePdfLibraries()
      ...
      .catch(function (err) {
        console.error(
          "[Mapa PDF] Error generando documento:",
          err
        );
        ...
        var friendly =
          detail &&
          /conexión|internet|cargaron/i.test(
            detail
          )
            ? detail
